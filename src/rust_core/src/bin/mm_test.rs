...
fn setup_market_data(binance: &mut MockExchange, coinbase: &mut MockExchange, kraken: &mut MockExchange) {
...
fn simulate_market_movement(exchange: &mut MockExchange, rng: &mut ThreadRng) {
    ...
    if let Some(best_bid) = book.get_best_bid() {
        ...
        book.add_order(1, best_bid + price_change, (10 + size_change).max(1) as u32, true);
    }
    
    if let Some(best_ask) = book.get_best_ask() {
        book.cancel_order(3);  // Cancel old best ask
        book.add_order(3, best_ask + price_change, (8 + size_change).max(1) as u32, false);
    }
}
...
fn main() {
    ...
    if let Some(quotes) = mm.update_quotes() {
        ...
        println!("  Size: {:.2} BTC", quotes.buy_quote.quantity as f64 / 100.0);
        println!("  Exchange: {}", quotes.buy_quote.target_exchange);
        
        println!("\nSell Quote:");
        println!("  Price: ${:.2}", quotes.sell_quote.price);
        println!("  Size: {:.2} BTC", quotes.sell_quote.quantity as f64 / 100.0);
    ...
    for i in 0..10 {
        ...
        if let Some(quotes) = mm.update_quotes() {
            ...
            if rng.gen::<f64>() < 0.3 {  // 30% fill rate
                if rng.gen::<f64>() < 0.5 {
                    // Fill buy quote
                    mm.on_quote_filled(&quotes.buy_quote, quotes.buy_quote.price, quotes.buy_quote.quantity);
                    println!("Trade {}: Bought {:.2} BTC @ ${:.2}", 
                             i + 1,
    ...
    println!("Within risk limits: {}", if mm.is_within_risk_limits() { "YES" } else { "NO" });
    ...
    if let Some(quotes) = mm.update_quotes() {
        ...
        if let Some(new_quotes) = mm.update_quotes() {
            ...
            println!("  Buy: ${:.2} (smaller size: {:.2} BTC)", 
                     new_quotes.buy_quote.price,
                     new_quotes.buy_quote.quantity as f64 / 100.0);
            println!("  Sell: ${:.2} (larger size: {:.2} BTC)", 
                     new_quotes.sell_quote.price,
                     new_quotes.sell_quote.quantity as f64 / 100.0);
        }
    }
